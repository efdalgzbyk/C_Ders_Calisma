# C_Ders_Calisma
📂 Proje İçeriği
Bu repo aşağıdaki konuları kapsamaktadır:

Dikdörtgenin alanı / çevresi = main1

İkinci dereceden denklemin kökleri = yakında

"Hello World" 10 kez yazdırma = main2

İlk 50 doğal sayının toplamı = main3

2⁴ hesabı = main4

İki sayının EKOK’u = main5

1’den n’e asal sayılar = main6

1’den n’e asal sayıların toplamı = main7

Armstrong sayısı kontrolü = main8

Armstrong sayıları listeleme = main10

Mükemmel sayı kontrolü = mian9

Mükemmel sayıları listeleme = main11

Strong number kontrolü = main12

Strong number listeleme = main13

Palindrom sayı kontrolü = main14

Seri toplamı: 1 − X²/2! + X⁴/4! … = yakında

Harmonik seri (1 + 1/2 + ... 1/n) = yakında

Floyd üçgeni = main16

Seri: 1 + x + x²/2! + ... = yakında

Seri: x − x³ + x⁵ − ... = yakında

Seri: 1 + 11 + 111 + ... = yakında

100–200 arası 9’a bölünebilen sayıların toplamı = main15

Onluk → ikilik çevirme (arraysiz) = yakında

İkilik → onluk çevirme (array, fonksiyon, while yok) = yakında

Pascal üçgeni = main17


🚀 Nasıl Çalıştırılır?

Her soru kendi C dosyasında yazılmıştır.
seçili dosyayı derleyicide çalıştırabilrisiniz.

🎯 Amaç

Bu proje, temel algoritma mantığını geliştirmek, 
C dilinde çözüm üretebilmek ve aynı zamanda akış şeması oluşturma pratiği yapmak için hazırlanmıştır.

📬 İletişim

Her türlü soru, öneri veya hata bildirimi için issue açabilirsiniz.
Sizde soruları görmek isterseniz #homework2 den bakabilirsiniz
